...
def verify_top_level(src_drive: str, dest_folder: str):
...
def copy_sdcard_files(src_drive: str, dest_folder: str, board: str, slot: int, max_passes: int = 3) -> bool:
    try:
        ...
        files = [f for f in os.listdir(src_drive) if os.path.isfile(os.path.join(src_drive, f))]
        if not files:
            mark_and_update(board, slot, "copy", "failed", "no files")
            return False
        ...
        for _attempt in range(1, max_passes + 1):
            if CANCEL_EVENT.is_set():
                mark_and_update(board, slot, "copy", "failed", "cancelled")
                return False
            _, _, missing, mismatched = verify_top_level(src_drive, dest_folder)
...
from PyQt5.QtWidgets import (
    ...
    QLineEdit, QRadioButton, QProgressBar, QAction, QMenuBar,
    QAbstractItemView, QScrollArea, QDialog, QDialogButtonBox, QFrame, QGridLayout
)
